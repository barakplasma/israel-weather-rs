...
pub fn get_israeli_weather_forecast() -> Result<ims_structs::LocationForecasts, serde_xml_rs::Error> {
    ...
    let path = cache
        ...
        .expect("failed to download forecast xml");

    assert!(path.is_file());

    let forecast_xml = std::fs::read_to_string(path).expect("failed to read forecast xml");

    let forecasts: Result<ims_structs::LocationForecasts, serde_xml_rs::Error> = from_str(&forecast_xml);
...
mod tests {
    ...
    fn transform_weather_code() {
        let xml = std::fs::read_to_string("./weather.xml").unwrap();
        ...
        let english = forecasts.location[0].location_data.forecast[0].weather_code_english.as_ref().unwrap();
        assert_eq!(english, "Clear");
    }
    ...
    fn transform_forecast_times_to_datetimes() {
        ...
        assert_eq!(time, "2023-06-25T03:00:00+00:00");
    }
}
